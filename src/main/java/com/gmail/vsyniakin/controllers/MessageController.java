...
public class MessageController {
    ...
    public String addMessage(@RequestParam(required = false) String text, @RequestParam long idr, @ModelAttribute UserAccount userAccount) {
        if (!text.isEmpty()) {
            Recipe recipe = recipeService.getByIdFetchMessages(idr);
            Message message = Message.addMessage(text, userAccountService.getByIdFetchMessages(userAccount.getId()), recipe);
    ...
    public ResponseEntity<List<Message>> getMessagesByRecipe(@RequestParam(required = false, defaultValue = "0") int page, @RequestParam long idr) {
        ...
        if (countMessages > start) {
            List<Message> messages = messageService.getMessagesByRecipeFetchUserAccountAndSortByDate(recipe, start, MESSAGES_IN_PAGE);
