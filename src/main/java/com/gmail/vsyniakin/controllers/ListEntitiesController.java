...
public class ListEntitiesController {
    ...
    public String getRecipes(@RequestParam(required = false, defaultValue = "0") int page, @RequestAttribute String pageView, Model model) {

        List<Recipe> recipes = recipeService.getRecipesDescByRating(0.0, page * COUNT_RECIPES_IN_PAGE, COUNT_RECIPES_IN_PAGE);
        model.addAttribute("recipes", recipes);
        int countOfRecipes = recipeService.count();
        int countOfPages = countOfRecipes / COUNT_RECIPES_IN_PAGE + (((countOfRecipes % COUNT_RECIPES_IN_PAGE) == 0) ? 0 : 1);
    ...
    public String getRecipesByName(@RequestParam(required = false, defaultValue = "0") int page, @RequestParam String name, @RequestAttribute String pageView, Model model) {

        int countOfRecipes = recipeService.count(name);
        int countOfPages = countOfRecipes / COUNT_ENTITIES_IN_PAGE + (((countOfRecipes % COUNT_ENTITIES_IN_PAGE) == 0) ? 0 : 1);
        ...
        List<Recipe> recipes = recipeService.getRecipesByName(name, page * COUNT_ENTITIES_IN_PAGE, COUNT_ENTITIES_IN_PAGE);
    ...
    public String getRecipesByParameters(@RequestAttribute(required = false) TypeRecipe type,
        ...
        List<Recipe> recipes = recipeService.getRecipesByParameters(type, difficulty, rating, timeMin, ingredients, page * COUNT_RECIPES_IN_PAGE, COUNT_RECIPES_IN_PAGE);
        model.addAttribute("recipes", recipes);
    ...
    public String getRecipesByUser(@RequestParam(required = false, defaultValue = "0") int page, @RequestParam long idu, Model model) {
        ...
        model.addAttribute("first", getFirstOfButtonGroup(page, countOfPages));
        model.addAttribute("last", getLastOfButtonGroup(page, countOfPages));
        List<Recipe> recipes = recipeService.getRecipesByUserAccount(userAcc, page * COUNT_RECIPES_IN_PAGE, COUNT_RECIPES_IN_PAGE);
        model.addAttribute("recipes", recipes);
        return "acc";
    }

    @RequestMapping("/get_messages_by_user")
    public String getMessagesByUser(@RequestParam long idu, @RequestParam(required = false, defaultValue = "0") int page, Model model) {
        UserAccount userAcc = userAccountService.getReference(idu);
        model.addAttribute("userAcc", userAcc);
        int countOfMessages = messageService.count(userAcc);
        int countOfPages = countOfMessages / COUNT_MESSAGES_IN_PAGE + (((countOfMessages % COUNT_MESSAGES_IN_PAGE) == 0) ? 0 : 1);
        model.addAttribute("countOfPages", countOfPages);
        model.addAttribute("page", page);
        model.addAttribute("first", getFirstOfButtonGroup(page, countOfPages));
        model.addAttribute("last", getLastOfButtonGroup(page, countOfPages));
        List<Message> messages = messageService.getMessagesByUserAccountFetchRecipeAndSortByDate(userAcc, page * COUNT_MESSAGES_IN_PAGE, COUNT_MESSAGES_IN_PAGE);
        model.addAttribute("messages", messages);
        return "acc";
    }

    @RequestMapping("/get_messages_by_text")
    public String getMessageByText(@RequestParam(required = false, defaultValue = "0") int page, @RequestParam String text, Model model) {

        int countOfMessages = messageService.count(text);
        int countOfPages = countOfMessages / COUNT_ENTITIES_IN_PAGE + (((countOfMessages % COUNT_ENTITIES_IN_PAGE) == 0) ? 0 : 1);
        model.addAttribute("countOfPages", countOfPages);
        model.addAttribute("page", page);
        model.addAttribute("first", getFirstOfButtonGroup(page, countOfPages));
        model.addAttribute("last", getLastOfButtonGroup(page, countOfPages));
        List<ViewModerationObject> viewModerationObjects = new ArrayList<>();
        List<Message> messages = messageService.getMessageByText(text, page * COUNT_ENTITIES_IN_PAGE, COUNT_ENTITIES_IN_PAGE);
        if (messages != null) {
    ...
    public String getEntitiesToModeration(@RequestParam(required = false, defaultValue = "0") int page, Model model) {
        ...
        int countOfPages = countOfEntities / COUNT_ENTITIES_IN_PAGE + (((countOfEntities % COUNT_ENTITIES_IN_PAGE) == 0) ? 0 : 1);
        ...
        if (page * COUNT_ENTITIES_IN_PAGE < countOfRecipes) {
            List<Recipe> recipes = recipeService.getRecipesByModeration(false, page * COUNT_ENTITIES_IN_PAGE, COUNT_ENTITIES_IN_PAGE);
            if (recipes != null) {
                ViewModerationObject.convertRecipesToListViewModeration(recipes, viewModerationObjects);
            }
            if (recipes.size() < COUNT_ENTITIES_IN_PAGE) {
                List<Message> messages = messageService.getMessagesByModeration(false, (page * COUNT_ENTITIES_IN_PAGE - countOfRecipes), (COUNT_ENTITIES_IN_PAGE - recipes.size()));
                if (messages != null) {
                    ViewModerationObject.convertMessagesToListViewModeration(messages, viewModerationObjects);
                }
            }
        } else {
            List<Message> messages = messageService.getMessagesByModeration(false, (page * COUNT_ENTITIES_IN_PAGE - countOfRecipes), COUNT_ENTITIES_IN_PAGE);
            if (messages != null) {
                ViewModerationObject.convertMessagesToListViewModeration(messages, viewModerationObjects);
            }
        }
