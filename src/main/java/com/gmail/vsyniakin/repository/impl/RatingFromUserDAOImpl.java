...
public class RatingFromUserDAOImpl implements RatingFromUserDAO {
    ...
    public RatingFromUser searchByRecipeAndUser(Recipe recipe, UserAccount userAccount) {
        TypedQuery<RatingFromUser> query = entityManager.createQuery("SELECT r FROM RatingFromUser r JOIN FETCH r.recipe JOIN FETCH r.userAccount WHERE r.recipe = :recipe AND r.userAccount = :userAccount", RatingFromUser.class);
