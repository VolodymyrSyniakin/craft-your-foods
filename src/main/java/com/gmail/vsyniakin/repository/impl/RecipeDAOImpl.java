...
public class RecipeDAOImpl implements RecipeDAO {
	...
	public Recipe getByIdFetchMessages(long id) {
		TypedQuery<Recipe> query = entityManager
				.createQuery("SELECT r FROM Recipe r LEFT JOIN FETCH r.messages WHERE r.id = :id", Recipe.class);
		query.setParameter("id", id);
		return query.getSingleResult();
	}
	...
	public Recipe getByIdFetchRatingFromUsers(long id) {
		TypedQuery<Recipe> query = entityManager
				.createQuery("SELECT r FROM Recipe r LEFT JOIN FETCH r.ratingFromUsers WHERE r.id = :id", Recipe.class);
		query.setParameter("id", id);
		return query.getSingleResult();
	}
	...
	public List<Recipe> getRecipesDescByRating(double minRating, int start, int count) {
		TypedQuery<Recipe> query = entityManager.createQuery("SELECT r FROM Recipe r ORDER BY r.rating DESC",
				Recipe.class);
	...
	public Recipe getByIdFetchImage(long id) {
		TypedQuery<Recipe> query = entityManager
				.createQuery("SELECT r FROM Recipe r LEFT JOIN FETCH r.image WHERE r.id = :id", Recipe.class);
		query.setParameter("id", id);
		return query.getSingleResult();
	}

	@Override
	public List<Recipe> getRecipesByParameters(TypeRecipe type, DifficultyRecipe difficulty, double rating, int timeMin,
	...
	private StringBuilder builderQuery(StringBuilder result, TypeRecipe type, DifficultyRecipe difficulty,
			double rating, int timeMin, List<Ingredient> ingredients) {
		int i = 0;
		if (type != null) {
	...
	private List<Object> parametersToList(TypeRecipe type, DifficultyRecipe difficulty, double rating, int timeMin,
	...
	public List<Recipe> getRecipesByUserAccount(UserAccount userAccount, int start, int count) {
		TypedQuery<Recipe> query = entityManager.createQuery("SELECT r FROM Recipe r WHERE r.userAccount=:userAccount",
				Recipe.class);
	...
	@Override
	public int count(TypeRecipe type, DifficultyRecipe difficulty, double rating, int timeMin,
			List<Ingredient> ingredients) {
		StringBuilder builderQuery = new StringBuilder();
	...
	public int count(UserAccount userAccount) {
		TypedQuery<Long> query = entityManager
				.createQuery("SELECT COUNT(r) FROM Recipe r WHERE r.userAccount = :userAccount", Long.class);
		query.setParameter("userAccount", userAccount);
		return query.getSingleResult().intValue();
	}
	...
	public List<Recipe> getRecipesByModeration(boolean moderation, int start, int count) {
		TypedQuery<Recipe> query = entityManager.createQuery("SELECT r FROM Recipe r WHERE r.moderation = :moderation",
				Recipe.class);
	...
	public int count(boolean moderation) {
		TypedQuery<Long> query = entityManager
				.createQuery("SELECT COUNT(r) FROM Recipe r WHERE r.moderation = :moderation", Long.class);
		query.setParameter("moderation", moderation);
		return query.getSingleResult().intValue();
	}
	...
	public List<Recipe> getRecipesByName(String name, int start, int count) {
		TypedQuery<Recipe> query = entityManager.createQuery("SELECT r FROM Recipe r WHERE r.name  LIKE :name",
				Recipe.class);
	...
	public int count(String name) {
		TypedQuery<Long> query = entityManager.createQuery("SELECT COUNT(r) FROM Recipe r WHERE r.name LIKE :name",
				Long.class);
		query.setParameter("name", "%" + name + "%");
		return query.getSingleResult().intValue();
	}
	...
	public List<Recipe> getRecipesDescByDate(int start, int count) {
		TypedQuery<Recipe> query = entityManager.createQuery("SELECT r FROM Recipe r ORDER BY r.date DESC",
				Recipe.class);
		query.setFirstResult(start);
		query.setMaxResults(count);
		return query.getResultList();
