...
public class MessageDAOImpl implements MessageDAO {
	...
	public List<Message> getMessagesByRecipeFetchUserAccountAndSortByDate(Recipe recipe, int start, int count) {
		TypedQuery<Message> query = entityManager.createQuery(
				"SELECT m FROM Message m LEFT JOIN FETCH m.userAccount WHERE m.recipe = :recipe ORDER BY m.date DESC",
				Message.class);
	...
	public int count(Recipe recipe) {
		TypedQuery<Long> query = entityManager.createQuery("SELECT COUNT(m) FROM Message m WHERE m.recipe = :recipe",
				Long.class);
		query.setParameter("recipe", recipe);
	...
	@Override
	public List<Message> getMessagesByUserAccountFetchRecipeAndSortByDate(UserAccount userAccount, int start,
			int count) {
		TypedQuery<Message> query = entityManager.createQuery(
				"SELECT m FROM Message m LEFT JOIN FETCH m.recipe WHERE m.userAccount = :userAccount ORDER BY m.date DESC",
				Message.class);
	...
	public int count(UserAccount userAccount) {
		TypedQuery<Long> query = entityManager
				.createQuery("SELECT COUNT(m) FROM Message m WHERE m.userAccount = :userAccount", Long.class);
		query.setParameter("userAccount", userAccount);
		return query.getSingleResult().intValue();
	}
	...
	public List<Message> getMessagesByModeration(boolean moderation, int start, int count) {
		TypedQuery<Message> query = entityManager
				.createQuery("SELECT m FROM Message m WHERE m.moderation = :moderation", Message.class);
	...
	public int count(boolean moderation) {
		TypedQuery<Long> query = entityManager
				.createQuery("SELECT COUNT(m) FROM Message m WHERE m.moderation = :moderation", Long.class);
		query.setParameter("moderation", moderation);
		return query.getSingleResult().intValue();
	}
	...
	public List<Message> getMessageByText(String text, int start, int count) {
		TypedQuery<Message> query = entityManager.createQuery("SELECT m FROM Message m WHERE m.text LIKE :text",
				Message.class);
	...
	public int count(String text) {
		TypedQuery<Long> query = entityManager.createQuery("SELECT COUNT(m) FROM Message m WHERE m.text LIKE :text",
				Long.class);
		query.setParameter("text", "%" + text + "%");
		return query.getSingleResult().intValue();
	}
